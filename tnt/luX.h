...
int LUX_factor( MaTRiX &A, VecToRSubscript &row, VecToRSubscript &col)
{
    assert(A.lbound() == 1);                // currently for 1-offset
    assert(row.lbound() == 1);              // vectors and matrices
    assert(col.lbound() == 1);              // vectors and matrices

    Subscript M = A.num_rows();
    Subscript N = A.num_cols();

    if (M == 0 || N==0) return 0;
    if (row.dim() != M) row.newsize(M);
    if (col.dim() != N) col.newsize(M);

    Subscript i=1,j=1,k=1;
    Subscript jp=0;
    Subscript jl=N;

    for (i=1; i<=M; i++) row(i) = i;
