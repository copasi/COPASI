...
TEST_CASE("1: load model, simulate, collect data", "[copasi][datahandler]")
{
  ...
  SECTION("brusselator example")
  {
    ...
    inv_handler.addDuringName(CCommonName("CN=Root,Model=The Brusselator"));  // this should not work and give a warning during compile, and stop it
    ...
    handler.addDuringName(CCommonName("CN=Root,Model=The Brusselator,Reference=Time"));
    handler.addDuringName(CCommonName("CN=Root,Model=The Brusselator,Vector=Compartments[compartment],Vector=Metabolites[X],Reference=Concentration"));
    handler.addDuringName(CCommonName("CN=Root,Model=The Brusselator,Vector=Compartments[compartment],Vector=Metabolites[Y],Reference=Concentration"));
    handler.addDuringName(CCommonName("CN=Root,Model=The Brusselator,Vector=Reactions[R1],Reference=Flux"));
    handler.addDuringName(CCommonName("CN=Root,Model=The Brusselator,Vector=Reactions[R2],Reference=Flux"));
    handler.addDuringName(CCommonName("CN=Root,Model=The Brusselator,Vector=Reactions[R3],Reference=Flux"));
    handler.addDuringName(CCommonName("CN=Root,Model=The Brusselator,Vector=Reactions[R4],Reference=Flux"));

    // add test to verify that array element references can be correctly resolved
    handler.addDuringName(CCommonName("CN=Root,Vector=TaskList[Metabolic Control Analysis],Method=MCA Method (Reder),Array=Scaled elasticities[(R1)][X]"));
...
TEST_CASE("ensure that data handler with function evaluations can be compiled", "[copasi][datahandler]")
{
  ...
  CDataHandler handler;
  handler.addDuringName(CCommonName("CN=Root,Vector=TaskList[Optimization],Problem=Optimization,Reference=Function Evaluations"));
  handler.addDuringName(CCommonName("CN=Root,Vector=TaskList[Optimization],Problem=Optimization,Reference=Best Value"));
  ...
  {
    auto & item = problem->addOptItem(CCommonName("CN=Root,Model=The Brusselator,Vector=Compartments[compartment],Vector=Metabolites[A],Reference=InitialConcentration"));
  ...
  {
    auto & item = problem->addOptItem(CCommonName("CN=Root,Model=The Brusselator,Vector=Compartments[compartment],Vector=Metabolites[B],Reference=InitialConcentration"));
    item.setLowerBound(0.0001);
    item.setUpperBound(100);
