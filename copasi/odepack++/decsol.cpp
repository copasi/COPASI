  ...
  for (k = 1; k <= i__1; ++k)
    {
      ...
      for (i__ = kp1; i__ <= i__2; ++i__)
        {
          if ((d__1 = a[i__ + k * a_dim1], fabs(d__1)) > (d__2 = a[m + k * a_dim1], fabs(d__2)))
            {
              m = i__;
            }
          /* L10: */
...
/* Subroutine */ int sol_(const integer * n,
                          const integer * ndim,
                          const doublereal * a,
                          doublereal * b,
                          const integer * ip)
  ...
  for (k = 1; k <= i__1; ++k)
    {
      ...
      for (i__ = kp1; i__ <= i__2; ++i__)
        {
          if ((d__1 = ar[i__ + k * ar_dim1], fabs(d__1)) + (d__2 = ai[i__ + k * ai_dim1], fabs(d__2)) > (d__3 = ar[m + k * ar_dim1],
                                                                                                         fabs(d__3))
...
L70:
  k = *n;
  if ((d__1 = ar[*n + *n * ar_dim1], fabs(d__1)) + (d__2 = ai[*n + *n * ai_dim1], fabs(d__2)) == 0.)
    {
      goto L80;
    }
  return 0;
  ...
  for (kb = 1; kb <= i__1; ++kb)
    {
      km1 = *n - kb;
      k = km1 + 1;
      den = ar[k + k * ar_dim1] * ar[k + k * ar_dim1] + ai[k + k * ai_dim1] * ai[k + k * ai_dim1];
...
L70:
  k = *n;
  if ((d__1 = ar[md + *n * ar_dim1], fabs(d__1)) + (d__2 = ai[md + *n * ai_dim1], fabs(d__2)) == 0.)
    {
      goto L80;
    }
  return 0;
...
L25:
  ...
  for (kb = 1; kb <= i__1; ++kb)
    {
      k = *n + 1 - kb;
      den = ar[md + k * ar_dim1] * ar[md + k * ar_dim1] + ai[md + k * ai_dim1] * ai[md + k * ai_dim1];
