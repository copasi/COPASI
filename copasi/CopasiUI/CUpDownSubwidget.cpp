/* Begin CVS Header
   $Source: /Volumes/Home/Users/shoops/cvs/copasi_dev/copasi/CopasiUI/Attic/CUpDownSubwidget.cpp,v $
   $Revision: 1.1 $
   $Name:  $
   $Author: ssahle $ 
   $Date: 2005/02/22 15:54:51 $
   End CVS Header */

/****************************************************************************
 ** Form implementation generated from reading ui file 'CUpDownSubwidget.ui'
 **
 ** Created: Di Feb 22 03:48:41 2005
 **      by: The User Interface Compiler ($Id: CUpDownSubwidget.cpp,v 1.1 2005/02/22 15:54:51 ssahle Exp $)
 **
 ** WARNING! All changes made in this file will be lost!
 ****************************************************************************/

#include "CUpDownSubwidget.h"

#include <qvariant.h>
#include <qpushbutton.h>
#include <qlayout.h>
#include <qtooltip.h>
#include <qwhatsthis.h>
#include <qimage.h>
#include <qpixmap.h>

#include "CUpDownSubwidget.ui.h"
static const unsigned char image0_data[] =
  {
    
  };

static const unsigned char image1_data[] =
  {
    
  };

static const unsigned char image2_data[] =
  {
    
  };

/*
 *  Constructs a CUpDownSubwidget as a child of 'parent', with the
 *  name 'name' and widget flags set to 'f'.
 */
CUpDownSubwidget::CUpDownSubwidget(QWidget* parent, const char* name, WFlags fl)
    : QWidget(parent, name, fl)
{
  QImage img;
  img.loadFromData(image0_data, sizeof(image0_data), "PNG");
  image0 = img;
  img.loadFromData(image1_data, sizeof(image1_data), "PNG");
  image1 = img;
  img.loadFromData(image2_data, sizeof(image2_data), "PNG");
  image2 = img;
  if (!name)
    setName("CUpDownSubwidget");
  setSizePolicy(QSizePolicy((QSizePolicy::SizeType)1, (QSizePolicy::SizeType)1, 0, 0, sizePolicy().hasHeightForWidth()));
  CUpDownSubwidgetLayout = new QVBoxLayout(this, 0, 0, "CUpDownSubwidgetLayout");

  layout = new QGridLayout(0, 1, 1, 3, 3, "layout");

  buttonDown = new QPushButton(this, "buttonDown");
  buttonDown->setMaximumSize(QSize(20, 20));
  buttonDown->setPixmap(image0);

  layout->addWidget(buttonDown, 1, 1);

  buttonUp = new QPushButton(this, "buttonUp");
  buttonUp->setMaximumSize(QSize(20, 20));
  buttonUp->setPixmap(image1);

  layout->addWidget(buttonUp, 0, 1);

  buttonDel = new QPushButton(this, "buttonDel");
  buttonDel->setMaximumSize(QSize(20, 20));
  buttonDel->setPixmap(image2);

  layout->addWidget(buttonDel, 0, 0);
  CUpDownSubwidgetLayout->addLayout(layout);
  spacer = new QSpacerItem(1, 1, QSizePolicy::Minimum, QSizePolicy::Expanding);
  CUpDownSubwidgetLayout->addItem(spacer);
  languageChange();
  resize(QSize(124, 105).expandedTo(minimumSizeHint()));
  clearWState(WState_Polished);

  // signals and slots connections
  connect(buttonDel, SIGNAL(clicked()), this, SLOT(slotDel()));
  connect(buttonDown, SIGNAL(clicked()), this, SLOT(slotDown()));
  connect(buttonUp, SIGNAL(clicked()), this, SLOT(slotUp()));
  init();
}

/*
 *  Destroys the object and frees any allocated resources
 */
CUpDownSubwidget::~CUpDownSubwidget()
{
  // no need to delete child widgets, Qt does it all for us
}

/*
 *  Sets the strings of the subwidgets using the current
 *  language.
 */
void CUpDownSubwidget::languageChange()
{
  setCaption(tr("Form2"));
  buttonDown->setText(QString::null);
  buttonUp->setText(QString::null);
  buttonDel->setText(QString::null);
}
