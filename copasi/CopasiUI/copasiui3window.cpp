...
MessageDialog::MessageDialog(QWidget* parent): QDialog(parent),
    mEscapeValue(Qt::NoButton),
    mReturnValue(Qt::NoButton)
{
  mpMainLayout = new QVBoxLayout(this);
  mpMainLayout->setAutoAdd(false);
  mpMessageView = new QScrollView(this);
  mpMainLayout->addWidget(mpMessageView);
  mpMessageBox = new QHBox(mpMessageView->viewport());
  mpMessageView->addChild(mpMessageBox);
  mpMessageBox->layout()->setAutoAdd(false);
  mpButtonBox = new QHBox(this);
  mpMainLayout->addWidget(mpButtonBox);
  mpButtonBox->layout()->setAutoAdd(false);
  static_cast<QBoxLayout*>(mpButtonBox->layout())->addStretch(1);
  mpButton0 = new QPushButton(mpButtonBox);
  static_cast<QBoxLayout*>(mpButtonBox->layout())->addWidget(mpButton0);
  mpButton1 = new QPushButton(mpButtonBox);
  static_cast<QBoxLayout*>(mpButtonBox->layout())->addWidget(mpButton1);
  mpButton2 = new QPushButton(mpButtonBox);
  static_cast<QBoxLayout*>(mpButtonBox->layout())->addWidget(mpButton2);
  mpButton3 = new QPushButton(mpButtonBox);
  static_cast<QBoxLayout*>(mpButtonBox->layout())->addWidget(mpButton3);
  static_cast<QBoxLayout*>(mpButtonBox->layout())->addStretch(1);
  mpButtonBox->setMargin(5);
  mpMainLayout->addWidget(mpButtonBox);
  mpIconLabel = new QLabel(mpMessageBox);
  mpIconLabel->setSizePolicy(QSizePolicy::Fixed, QSizePolicy::MinimumExpanding);
  mpIconLabel->setAlignment(Qt::AlignTop | Qt::AlignHCenter);
  mpIconLabel->setMargin(5);
  mpMessageLabel = new QLabel(mpMessageBox);
  mpMessageLabel->setSizePolicy(QSizePolicy::MinimumExpanding, QSizePolicy::MinimumExpanding);
  mpMessageLabel->setAlignment(Qt::AlignTop | Qt::AlignLeft);
  mpMessageLabel->setMargin(0);
  mpMessageBox->setMargin(0);
  static_cast<QBoxLayout*>(mpMessageBox->layout())->addWidget(mpIconLabel);
  static_cast<QBoxLayout*>(mpMessageBox->layout())->addWidget(mpMessageLabel);
  connect(this->mpButton0, SIGNAL(clicked()), this, SLOT(slotButton0_clicked()));
  connect(this->mpButton1, SIGNAL(clicked()), this, SLOT(slotButton1_clicked()));
  connect(this->mpButton2, SIGNAL(clicked()), this, SLOT(slotButton2_clicked()));
  connect(this->mpButton3, SIGNAL(clicked()), this, SLOT(slotButton3_clicked()));
}

void MessageDialog::slotButton0_clicked()
{
  this->mReturnValue = mButton0_Type;
  QDialog::accept();
}

void MessageDialog::slotButton1_clicked()
{
  this->mReturnValue = mButton1_Type;
  QDialog::accept();
}

void MessageDialog::slotButton2_clicked()
{
  this->mReturnValue = mButton2_Type;
  QDialog::accept();
}

void MessageDialog::slotButton3_clicked()
{
  this->mReturnValue = mButton3_Type;
  QDialog::accept();
}

MessageDialog::~MessageDialog(){}

int MessageDialog::warning(QWidget * parent, const QString & caption,
                           const QString & text, int button0, int button1,
                           int button2 , int button3)
{
  MessageDialog message(parent);
  message.setCaption(caption);
  message.mpMessageLabel->setText(text);
  message.mpIconLabel->setPixmap(QMessageBox::standardIcon(QMessageBox::Warning));
  // set the properties in reverse order so that the first
  // button to have the default flag set will be the default
  // button
  message.mButton3_Type = message.setButtonProperties(message.mpButton3, button3);
  message.mButton2_Type = message.setButtonProperties(message.mpButton2, button2);
  message.mButton1_Type = message.setButtonProperties(message.mpButton1, button1);
  message.mButton0_Type = message.setButtonProperties(message.mpButton0, button0);
  message.exec();
  return message.mReturnValue;
}

int MessageDialog::setButtonProperties(QPushButton* pButton, int properties)
{
  if (properties & QMessageBox::Default)
    {
      pButton->setDefault(true);
    }
  // clear the default and the escape flag to get the button enum
  int type = (properties & (~(QMessageBox::Default | QMessageBox::Escape)));
  if (properties & QMessageBox::Escape)
    {
      this->mEscapeValue = type;
    }
  switch (type)
    {
    case QMessageBox::Ok:
    ...
    case QMessageBox::Abort:
      pButton->setText("Abort");
      break;
    case QMessageBox::Retry:
      pButton->setText("Retry");
      break;
    case QMessageBox::Ignore:
      pButton->setText("Ignore");
      break;
    case QMessageBox::YesAll:
      pButton->setText("Yes to all");
      break;
    case QMessageBox::NoAll:
      pButton->setText("No to all");
      break;
  ...
  if (msg_pending.getNumber() != MCCopasiMessage + 1)
    {
      ...
      MessageDialog::warning(this, QString("File Warning"), text,
                             QMessageBox::Ok, QMessageBox::NoButton, QMessageBox::NoButton);
