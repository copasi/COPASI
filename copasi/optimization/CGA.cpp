...
int CGA::optimise()
{
 ...
 if(argc<4)
  {
   cout<<endl<<"Usage: ga mParamNum mini_bound maxi_bond"<<endl;
  exit(1);
  }
 ...
 //Ga_10param.setMin(-10);
 //Ga_10param.setMax(10);

 // cout << "tmp1 = "<<tmp1<<", tmp2= "<<tmp2<<", tmp3 = "<<tmp3<<endl;
 cout << "mMin = "<<mMin<<", mMax= "<<mMax<<", mParamNum = "<<mParamNum<<endl;
 cout << "mMin = "<<Ga_10param.mMin<<", mMax= "<<Ga_10param.mMax<<", mParamNum = "<<Ga_10param.mParamNum<<endl;

//Initialise the random number generator with time
 Ga_10param.initOptRandom();

 // initialise the variance for mutations
 Ga_10param.setMutVar(0.1);
 ...
 bx = Ga_10param.getBestCandidate();
 ...
 //finalout<<"Generation\t"<<"Best candidate value for object function\t"<<"Display "<<Ga_10param.getParamNum()<<" parameters"<<endl;
 //finalout<<endl;
 //finalout.close();

 cout<<"-----------------------------best result at each generation---------------------"<<endl;
 cout<<"Generation\t"<<"Best candidate value for object function\t"<<"Display "<<Ga_10param.getParamNum()<<" parameters"<<endl;
 cout<<endl;

int psize = Ga_10param.getPopSize();

// ITERATE FOR gener GENERATIONS

for( i=0; i<Ga_10param.getGeneration(); i++ )
{
  cout<<endl;
  cout<<"GA is processing at generation "<<i<<endl;

  Ga_10param.dumpData( i );
  ...
  // get the index of the fittest
 // mBest = fittest();
 Ga_10param.setBest(Ga_10param.fittest());
  if(Ga_10param.getBestCandidate()!= bx )
  {
   last_update = i;
   bx = Ga_10param.getBestCandidate();
  }
