  ...
  for (i = 0; i < mVariableSize; i++)
    {
      ...
      pointInParameterDomain &= (OptItem.getStartValue() == mCurrent[i]);
  ...
  for (j = 0; j < mVariableSize && proceed(); ++j)
    {
      ...
      if (mValue[j] < getBestValue())
        setSolution(mValue[j], mCurrent, true);
