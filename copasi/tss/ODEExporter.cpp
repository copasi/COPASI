 ...
 **
 */
bool ODEExporter::exportMathModel(const CModel* copasiModel, std::string mmasciiFilename, std::string Filter, bool overwriteFile)
{
...
bool ODEExporter::preprocess(const CModel* copasiModel)
{
...
bool ODEExporter::exportTitleData(const CModel* copasiModel, std::ofstream & outFile)
{return true;}

/* export metabolites */
bool ODEExporter::exportMetabolites(const CModel* copasiModel)
{
...
bool ODEExporter::exportCompartements(const CModel* copasiModel)
{
  const CCopasiVector< CCompartment > & comps = copasiModel->getCompartments();
...
bool ODEExporter::exportModelValues(const CModel* copasiModel)
{
  const CCopasiVector< CModelValue > & modvals = copasiModel->getModelValues();
...
bool ODEExporter::exportModelValuesExpressions(const CModel* copasiModel, std::map< std::string, std::string > & equations)
{
  const CCopasiVector< CModelValue > & modvals = copasiModel->getModelValues();
...
std::string ODEExporter::isExpressionODEExporterCompatible(CModelEntity * modval, const CExpression* pExpression)
{
...
std::string ODEExporter::exportExpression(const CExpression* pExpression, std::map< std::string, std::string > & equations)
{
...
bool ODEExporter::exportReacParamsAndFuncs(const CModel* copasiModel, std::map< std::string, std::string > & equations)
{
...
/* export ODEs  */
bool ODEExporter::exportODEs(const CModel* copasiModel, std::map< std::string, std::string > & equations)
{
  const CCopasiVector< CMetab > & metabs = copasiModel->getMetabolitesX();
  unsigned C_INT32 indep_size = copasiModel->getNumIndependentMetabs();
...
bool ODEExporter::exportClosingData(const CModel* copasiModel, std::ofstream & outFile)
{return true;}

void ODEExporter::setReservedNames()
{return;}

std::string ODEExporter::translateTimeVariableName()
{return " ";}

std::string ODEExporter::translateObjectName(const std::string & realName)
{return " ";}

std::string ODEExporter::setODEName(const std::string & objName)
{return " ";}

bool ODEExporter::exportSingleObject(std::ostringstream & which, std::string & name, std::string & expression, std::string & comments)
{return true;}

bool ODEExporter::exportSingleMetabolite(const CMetab* metab, std::string & expression, std::string & comments)
{return true;}

bool ODEExporter::exportSingleCompartement(const CCompartment* comp, std::string & expression, std::string & comments)
{return true;}

bool ODEExporter::exportSingleModVal(const CModelValue* modval, std::string & expression, std::string & comments)
{return true;}

bool ODEExporter::exportSingleParameter(const CCopasiParameter* param, std::string & expression, std::string & comments)
{return true;}

bool ODEExporter::exportKineticFunction (CReaction* reac)
{
...
bool ODEExporter::exportSingleFunction(CEvaluationNode* pNode, const CReaction *reac, unsigned C_INT32 &index)
{
  if (pNode)
    {
...
std::string ODEExporter::getDisplayFunctionString(CFunction * func)
{return " ";}

std::string ODEExporter::getDisplayExpressionString(CExpression * tmp)
{return " ";}

bool ODEExporter::exportSingleODE(const CModelEntity* mentity, std::string & equation, std::string & comments)
{return true;}

bool ODEExporter::exportKineticFunctionGroup (const CModel* copasiModel)
{return true;}

std::string ODEExporter::KineticFunction2ODEmember(const CReaction *reac)
{return " ";}

std::string ODEExporter::exportTitleString(const unsigned C_INT32 tmp)
{return " ";}

std::string ODEExporter::exportClosingString(const unsigned C_INT32 tmp)
{return " ";}

/**
 **       This method tests if a string consists only of whitespace characters
 **/
bool ODEExporter::isEmptyString(std::string & str)
{return (str.find_first_not_of(" \n\t\r") == std::string::npos);}

/**
 **      This method modifies the export tree of the function for internal calls of Mass Action
 **/
void ODEExporter::modifyTreeForMassAction(CFunction* tmpfunc)
{
  CFunctionDB* pFunctionDB = CCopasiDataModel::Global->getFunctionList();
          ...
          if (callfunc->getType() == CEvaluationTree::MassAction)
            {
              ...
              ODEExporter::assembleSubTreeForMassAction(newNode1, child1, child2);
              ...
              if (callfunc->getObjectName() == "Mass action (reversible)")
                {
                  ...
                  ODEExporter::assembleSubTreeForMassAction(newNode2, child3, child4);
 ...
 **/
void ODEExporter::assembleSubTreeForMassAction(CEvaluationNode* newNode, CEvaluationNode* child1, CEvaluationNode* child2)
{
