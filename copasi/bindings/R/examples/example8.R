...
if is.null(model) {
    ...
    CModel_applyInitialValues(model)
    ...
    CModel_calculateJacobian(model,jacobian, 1e-12, 1.0)
    ...
    while( i < len(nameVector)) {
        ...
        while (j < len(nameVector)) {
            printf("%7.3f", FloatMatrix_get(jacobian,i,j))
            j <- j + 1
        }
    ...
    # in a similar way
    CModel_calculateJacobianX(model,jacobian, 1e-12, 1.0)
    ...
    i <- 0
    while (i < iMax) {
       printf("%7s", CCopasiObject_getObjectName(CStateTemplate_getIndependent(stateTemplate,i)))
       + <- i + 1
    }
