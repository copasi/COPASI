...
CChemEq_addMetabolite(chemEq,CModelEntity_getKey(S), 1.0, "SUBSTRATE")
# P is a product with stoichiometry 1
CChemEq_addMetabolite(chemEq,CModelEntity_getKey(P), 1.0, "PRODUCT")
...
CModelEntity_setStatus(MV,"FIXED")
...
CFunction_setReversible(func,"TriFALSE")
...
param <- CFunctionParameters_getParameter(variables,index)
CFunctionParameter_setUsage(param,"SUBSTRATE")

# set the rate law for the reaction
CReaction_setFunction(reaction,func)
...
CReaction_setParameterMapping(reaction,"substrate", CModelEntity_getKey(S))
CReaction_setParameterMapping(reaction,"temp", CModelENtity_getKey(MV))
...
CCopasiDataModel_saveModel(dataModel,"example7.cps", TRUE)
