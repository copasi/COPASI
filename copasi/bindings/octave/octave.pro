...
contains(BUILD_OS,Linux){
 ...
 QMAKE_POST_LINK += ln -sf libCopasiOctave.so _COPASI.so
...
contains(BUILD_OS, Darwin) {
  ...
  QMAKE_POST_LINK += ln -sf libCopasiOctave.dylib _COPASI.so
}
...
!isEmpty(SWIG_PATH){
    ...
    !contains(BUILD_OS, WIN32){
      wrapper_source.target = copasi_wrapper.cpp
      wrapper_source.depends = $$SWIG_INTERFACE_FILES octave.i local.cpp
      wrapper_source.commands = $(DEL_FILE) $$wrapper_source.target ; $$SWIG_PATH/bin/swig $$DEFINE_COMMANDLINE -I../.. -c++ -octave -o $$wrapper_source.target octave.i
  
      #wrapper.target = COPASI_R.so
      #wrapper.depends = $wrapper_source.target
      #wrapper.commands = $(DEL_FILE) $$wrapper_source.target ; $$SWIG_PATH/bin/swig $$DEFINE_COMMANDLINE -I../.. -c++ -octave -o $$wrapper_source.target octave.i; mkoctfile $wrapper_source.target 
...
QMAKE_CLEAN += COPASI.m

# we set the compiler to the MKOCTFILE variable if it is set, otherwise we assume that mkoctfile is in the path
!isEmpty(MKOCTFILE_BIN){
  QMAKE_LINK_SHLIB_CMD = $$MKOCTFILE_BIN $$wrapper_source.target $(INCPATH) $(DEFINES)
} else {
  QMAKE_LINK_SHLIB_CMD = mkoctfile $$wrapper_source.target $(INCPATH) $(DEFINES)
}

# unset all compiler flags since we let mkoctfile handle this
# QMAKE_CXXFLAGS = ""
