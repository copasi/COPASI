...
void print_annotated_matrix(const std::string & title, const CArrayAnnotation & annotated_matrix)
{
  std::cout << title << std::endl;
  std::cout << "==========" << std::endl;

  CCopasiAbstractArray::index_type size = annotated_matrix.size();
      ...
      for (size_t i = 0; i < columns; ++i)
        {
          ...
          const CCopasiObject * pObject = CObjectInterface::DataObject(annotated_matrix.getObject(StringPrint("[%d][%d]", j, i)));
