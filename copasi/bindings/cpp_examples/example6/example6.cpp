...
int main()
{
  ...
  if (pTrajectoryTask == NULL)
    {
      ...
      CCopasiDataModel::Global->getTaskList()->add(pTrajectoryTask);
    }
  ...
  CReaction* pReaction = CCopasiDataModel::Global->getModel()->getReactions()[0];
  ...
  pReaction = CCopasiDataModel::Global->getModel()->getReactions()[1];
  // we know that it is an irreversible mass action, so there is one
  // parameter
  assert(pReaction->getParameters().size() == 1);
  assert(pReaction->isLocalParameter(0));
  ...
  try
    {
      ...
      result = pFitTask->initialize(CCopasiTask::OUTPUT_COMPLETE, CCopasiDataModel::Global, NULL);
