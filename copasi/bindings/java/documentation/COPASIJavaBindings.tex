...
The second argument determines if the task generates output from  reports if any have been defined for the task. Possible values for the second argument are \textbf{CCopasiTask.NO\_OUTPUT} which tells the task not to generate output and \textbf{CCopasiTask.OUTPUT\COMPLETE} which means that the task should create output as defined in the associated report (see section \ref{CreatingReports}). 
