 ...
 * index and returnes the metabolite at that index.
 */
MetabVectorNS getMetabolites();

/**
 * Adds the given metabolite to the compartment.
 * If the operation succeeded, true is returned, else fale.
 */
 ...
 * role is the role of the metabolite and it can bei either
 ...
 * can be aquired with the get method. The only argument to
 ...
 * columns. This inclues the column for time.
 */
long getNumVariables();

/**
 * Gets the value for a specific row and column of 
 * the result. The rwos are the individual time steps 
 ...
 * of the model can be aquired by this method.
 */
...
/**
 * Returns the Jakobian matrix for the reduced model 
...
/**
 * Returns the annotated Jakobian matrix for the reduced model 
 * (removed mass conservations).
 */
 ...
 * of the model.
 * A value of false means always use the origininal initial
...
Strng objectiveFunction=object.getCN().getString();
...
\begin{lstlisting}
...
CReportDefinitionVector* reports =
   CCopasiDataModel.getGlobal().getReportDefinitionList();
CReportDefinition* report =
   reports.createReportDefinition("Report", "metab concentrations");
...
body.add(new CRegisteredObjectName(timeObject.getCN()));
unsigned int i,iMax=model.getNumMetabs() 
String ref=new String("Reference=Concentration");
CCopasiObject refObject=null;
for (i = 0; i < iMax;++i)
  {
     refObject=model.getMetabolite(i).getObject(ref);
     item=new CRegisteredObjectName(refObject.getCN());
     body.add(item);
  }
