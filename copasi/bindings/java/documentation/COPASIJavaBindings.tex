...
\textit{CCopasiDataModel} contains the actual model which can be obtained with the method \textit{getModel()}, the defined tasks which can be obtained with the method \textit{getTaskList()}, the reports which are obtained via the method \textit{getReportTemplateList()} and the function database which can be obtained with the \textit{getFunctionList()}.
...
In COPASI reports are represented by the class \textit{CReport}. A report stored a so called report template that specifies what is being written and it contains a filename that determines where the report is written to.
...
To set the report template of a report instance, use the method \textit{setReportTemplate}. The only argument to the call is an instance of class \textit{CReportTemplate}.
A report template can work in two modes. In the default mode, the report template is a table and all objects that are added to the report template are written separated by a user definable separator.
In the second mode a report template consists of three parts a header, a body and a footer. The part that is specified as the header is written before the associated task is executed, the body is executed for each step of the associated task and the footer is finally written after the task has finished.
Each part is a container that holds zero or more report template items. To get the individual containers, the \textit{CReportTemplate} class has the methods \textit{getTableAddr} if the report template is in table mode or \textit{getHeaderAddr}, \textit{getBodyAddr} and \textit{getFooterAddr} if it is not in table mode.
To specify whether a report template is to run in table mode, you can use the method \textit{setIsTable} which takes a boolean value that determines whether the report template is a table or not. The flag can be queried with the \textit{isTable} method.
...
If the registered object name should represent a model entity, the string has to be the common name of the object. If you want to add an element the report that represents a simple stirng, you have to create an instance of \textit{CCopasiStaticString} by calling the constructor of this class with the string that you want to add to the report template. Once you have an instance of \textit{CCopasiStaticString}, you can create an instance of \textit{CRegisteredObjectName} the same way as it is done for other objects, you take the string representation of the common name of your instance of \textit{CCopasiStaticString} and use it as the argument to the constructor of \textit{CRegisteredObjectName}.

In COPASI the reports templates are stored in the global instance of \textit{CCopasiDataModel}. To get this list use the method \textit{getReportTemplateList} on the global instance of \textit{CCopasiDataModel}.

\begin{verbatim}
CCopasiDataModel.getGlobal().getReportTemplateList()
\end{verbatim}

The returned object is a container that stores all report templates. To create a new report template, use \textit{createReportTemplates} which is a method of the container. The two arguments to the call are the name of the report template and a description. Once you have a report template object, you can use \textit{setTaskType} to set the type of task the report is intended for. The method takes one argument which is one of the task types defined in \textit{CCopasiTask}, e.g. \textit{CCopasiTask.timeCourse} for the time course task.
Next you can fill the tree sections (header, body, footer) of the report template or the table  with whatever has to be written to the output.
Once the report template is finished, we have to tell the task to use it. This is done through the tasks report object which can be acquire with the \textit{getReport()} method of \textit{CCopasiTask}. The returned report object has a method \textit{setReportTemplate} which takes the report template object as its argument.
...
CReportTemplateVector reportDefs=global.getReportTemplateList();
CReportTemplate repDef=reportDefs.createReportTemplate("htmlConc",
