...
In the description of the individual tasks and in the accompanying examples, we have always used \textbf{CCopasiTask.NO\_OUTPUT} as the first argument to the task which meant that we didn't want the task to create reports even if reports have been defined for the task. Now we naturally do want the task to create output, so we now use \textbf{CCopasiTask.OUTPUT\_COMPLETE} instead when we call \textit{initialize} on the task before running it.
