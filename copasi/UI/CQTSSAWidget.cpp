...
bool CQTSSAWidget::saveTaskProtected()
{
  ...
  if (pTssaProblem->getStepSize() != mpEditIntervalSize->text().toDouble())
    ...
    {
      pTssaProblem->setStepNumber(mpEditIntervals->text().toInt());
      mChanged = true;
    }
...
bool CQTSSAWidget::taskFinishedEvent()
{
  ...
  mpListView->switchToOtherWidget(ListViews::WidgetType::TimeScaleSeparationAnalysisResult, CRegisteredCommonName()); //change to the results window

  return true;
}

bool CQTSSAWidget::updateProtected(ListViews::ObjectType objectType, ListViews::Action action, const CRegisteredCommonName & cn)
{
