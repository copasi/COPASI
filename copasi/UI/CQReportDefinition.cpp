          ...
          for (i = 0; i < imax; ++i)
            {
              ...
              if (name.getObjectType() != "Separator")
                mpTableList->addItem(new CQReportListItem(name));
            }
...
void CQReportDefinition::slotAddSeparator()
{
  ...
  if (selected.empty())
    {
      current->addItem(new CQReportListItem(Separator.getCN()));
    }
  else
    {
      current->insertItem(current->row(selected.first()), new CQReportListItem(Separator.getCN()));
    }
