  ...
  for (int row = 0; row < rows; ++row)
    {
      ...
      name += TO_UTF8(QString::number(rowCount() + 1));
      (*CCopasiRootContainer::getDatamodelList())[0]->getModel()->createReaction(name);
    }
...
bool CQReactionDM::removeRows(int position, int rows, const QModelIndex&)
{
  ...
  for (int row = 0; row < rows; ++row)
    {
      (*CCopasiRootContainer::getDatamodelList())[0]->getModel()->removeReaction(row);
    }
