...
void copasi::COptionParser::finalize (void) {
	...
	switch (openum_) {
		...
		throw option_error("missing value for 'export-task' option");
		...
		throw option_error("missing value for 'import-task' option");
	    case option_License:
...
void copasi::COptionParser::parse_long_option (const char *option, int position, opsource source) {
    option = expand_long_name(option);
		...
		if (locations_.ExportTaskSpec) {
		    throw option_error("the 'export-task' option is only allowed once");
		}
		...
		if (locations_.ImportTaskSpec) {
		    throw option_error("the 'import-task' option is only allowed once");
		}
