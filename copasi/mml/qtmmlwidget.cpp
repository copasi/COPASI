...
class MmlNode : public Mml
  ...
  public:
    ...
    MmlNode *nextSybling() const
      {return m_next_sybling;}
    MmlNode *previousSybling() const
      {return m_previous_sybling;}
    MmlNode *lastSybling() const;
    MmlNode *firstSybling() const;
    bool isLastSybling() const
      {return m_next_sybling == 0;}
    bool isFirstSybling() const
      {return m_previous_sybling == 0;}
...
static QRect boundingRect(const QFontMetrics &fm, const QString &s);
...
void MmlDocument::_dump(const MmlNode *node, QString &indent) const
  {
    ...
    for (; child != 0; child = child->nextSybling())
      _dump(child, indent);
                              ...
                              QString *errorMsg)
{
  ...
  Q_ASSERT(new_node->parent() == 0
           && new_node->nextSybling() == 0
           && new_node->previousSybling() == 0);
  ...
  if (parent == 0)
    {
      if (m_root_node == 0)
        ...
        {
          MmlNode *n = m_root_node->lastSybling();
          n->m_next_sybling = new_node;
          new_node->m_previous_sybling = n;
        }
    }
  else
    {
      ...
      if (parent->hasChildNodes())
        {
          MmlNode *n = parent->firstChild()->lastSybling();
          n->m_next_sybling = new_node;
          new_node->m_previous_sybling = n;
        }
...
void MmlDocument::insertOperator(MmlNode *node, const QString &text)
{
  ...
  Q_ASSERT(insertChild(node, mo_node, 0));
  Q_ASSERT(insertChild(mo_node, text_node, 0));
}
      ...
      for (int i = 0; i < child_cnt; ++i)
        {
          ...
          if (i < child_cnt - 1 && mml_type == MfencedNode)
            {
              QChar separator;
              ...
              insertOperator(mml_node, separator);
            }
        }
...
MmlNode *MmlNode::lastSybling() const
  {
    const MmlNode *n = this;
    while (!n->isLastSybling())
      n = n->nextSybling();
    return const_cast<MmlNode*>(n);
  }

MmlNode *MmlNode::firstSybling() const
  {
    const MmlNode *n = this;
    while (!n->isFirstSybling())
      n = n->previousSybling();
    return const_cast<MmlNode*>(n);
  }
...
MmlNode *MmlMfracNode::denominator() const
  {
    MmlNode *node = numerator()->nextSybling();
    Q_ASSERT(node != 0);
    return node;
  }
...
MmlNode *MmlMsubsupNode::superscript() const
  {
    MmlNode *sup = subscript()->nextSybling();
    Q_ASSERT(sup != 0);
    return sup;
  }
...
Mml::FormType MmlMoNode::form() const
  {
    ...
    // Default heuristic.
    if (firstSybling() == (MmlNode*)this && lastSybling() != (MmlNode*)this)
      return PrefixForm;
    else if (lastSybling() == (MmlNode*)this && firstSybling() != (MmlNode*)this)
      return PostfixForm;
...
void MmlMoNode::stretch()
{
  ...
  if (m_oper_spec->stretch_dir == OperSpec::HStretch
      ...
      && (nextSybling() != 0 || previousSybling() != 0))
    return;
...
int MmlMoNode::lspace() const
  {
    ...
    if (parent() == 0
        ...
        || (previousSybling() == 0 && nextSybling() == 0))
      return 0;
    else
                             ...
                             int col_spc)
{
  ...
  int col_offset = 0;
  uint colnum = 0;
  for (; child != 0; child = child->nextSybling(), ++colnum)
    {
...
{
  MmlNode *syb = previousSybling();

  uint i = 0;
  for (; syb != 0; syb = syb->previousSybling())
    ++i;
...
{
  MmlNode *row = parent()->previousSybling();

  uint i = 0;
  for (; row != 0; row = row->previousSybling())
    ++i;
...
void MmlMunderNode::layoutSymbol()
{
  MmlNode *base = firstChild();
  Q_ASSERT(base != 0);
  MmlNode *under = base->nextSybling();
  Q_ASSERT(under != 0);
...
void MmlMunderoverNode::layoutSymbol()
{
  ...
  MmlNode *under = base->nextSybling();
  Q_ASSERT(under != 0);
  MmlNode *over = under->nextSybling();
  Q_ASSERT(over != 0);
...
/*!
    Constructs a QtMmlWidget object. The \a parent and \a name
...
void QtMmlWidget::drawContents(QPainter *p)
{
  QSize s = m_doc->size();
