/* Begin CVS Header
   $Source: /Volumes/Home/Users/shoops/cvs/copasi_dev/copasi/output/Attic/CDatum.cpp,v $
   $Revision: 1.34 $
   $Name:  $
   $Author: ssahle $ 
   $Date: 2004/06/22 16:11:44 $
   End CVS Header */
...
CDatum::CDatum(const std::string& title,
    ...
    mpValue(value),
    mType(type),
    mObjectName(((CCopasiObject *)this)->getObjectName())
{}
...
//const std::string & CDatum::getName() const {return mObjectName;}
...
/*C_INT32 CDatum::save(CWriteConfig & configbuffer)
{
  ...
  Type = getObjectType(mObjectName);
  ...
  switch (Type)
    ...
    case D_EIGVR:
    case D_EIGVI:
      IStr = getObjectIStr(mObjectName, 0);
    ...
    case D_FCC:
    case D_EIG:
      IStr = getObjectIStr(mObjectName, 1);
      Fail = configbuffer.setVariable("I", "string", &IStr);
      if (Fail)
        return Fail;
 
      JStr = getObjectJStr(mObjectName);
...
void CDatum::createObjectString(const std::string& model, const std::string& IStr, const std::string& JStr, C_INT32 Type)
{
  ...
  mObjectName = "";
  // Create CModel part
  mObjectName.append("CModel = \"");
  //mObjectName.append(Copasi->Model->getTitle());
  mObjectName.append(model);
  mObjectName.append("\", ");
  ...
  switch (Type)
    ...
    case D_EIGVR:
    case D_EIGVI:
      mObjectName.append("CMetab = \"" + IStr + "\"");
      break;
    case D_KIN:                        // Fall through as all have mI and mJ
    case D_ELAST:
    case D_CCC:
  ...
  if (!member.length())
    {
      ...
      mObjectName.append(", CMember = \"");
      mObjectName.append("\"");
    }
  else
    {
      // other types like D_TCONC
      mObjectName.append(", CMember = \"" + member + "\"");
    }
}
...
void CDatum::compileDatum(CModel *Model, CState *state, CSteadyStateTask *soln)
{
  ...
  switch (Type)
    ...
    case D_UFUNC:                    // user functions
      ...
      if (Index == -1)
        break;
      fatalError(); //pFunct = Copasi->pUDFunctionDB->getFunctions()[Index];
    ...
    case D_ICONC:                      // Fall through as all have mI but no mJ
      IStr = getObjectIStr(mObjectName, 0);
      Index = Model->findMetabByName(IStr); //TODO
      if (Index == -1)
        break;
...
;
    ...
    case D_TFLUX:
      IStr = getObjectIStr(mObjectName, 0);
      Index = Model->getReactions().getIndex(IStr);
      if (Index == -1)
        break;
    ...
    case D_VOL:
      IStr = getObjectIStr(mObjectName, 0);
      Index = Model->getCompartments().getIndex(IStr);
      if (Index == -1)
        break;
...
int CDatum::FindUDFunct(std::string title)
{
  ...
  /*for (i = 0; i < Copasi->pUDFunctionDB->getItems(); i++)
    if (Copasi->pUDFunctionDB->getFunctions()[i]->getObjectName() == title)
      return i;*/
