...
#ifndef _statslib_qinvgauss_HPP
#define _statslib_qinvgauss_HPP
 ...
 *
 * Example:
 * \code{.cpp} stats::qinvgauss(0.5,1.0,2.0); \endcode
 */

template<typename T1, typename T2, typename T3>
statslib_constexpr
common_return_t<T1,T2,T3>
qinvgauss(const T1 p, const T2 mu_par, const T3 lambda_par) noexcept;

/**
 * Quantile function of the Standard inverse Gaussian distribution
 *
 * @param p a real-valued input.
 *
 * @return the quantile function evaluated at \c p.
 *
 * Example:
 * \code{.cpp} stats::qinvgauss(0.5); \endcode
 */

template<typename T>
statslib_constexpr
return_t<T>
qinvgauss(const T p) noexcept;

//
// vector/matrix input

/**
 * @brief Quantile function of the inverse Gaussian distribution
 *
